    ...
    return (
        <a href={`https://www.youtube.com/watch?v=${video.id}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 group">
            ...
            <div className="overflow-hidden">
                <p className="text-xs font-semibold text-white line-clamp-2 group-hover:text-indigo-400 transition-colors custom-tooltip" title="">
                    {video.title}
                    <span className="tooltip-text">{video.title}</span>
                </p>
...
export const ComparisonDashboard: React.FC<ComparisonDashboardProps> = ({ group, data, onBack, dateRange, onDateRangeChange, isHighQuotaFeaturesEnabled, onToggleHighQuotaFeatures }) => {
    ...
    const handleSetPreset = (days: number | 'thisMonth' | 'lastMonth') => {
        // FIX: Changed 'end' to be mutable to allow for reassignment in the 'lastMonth' case.
        let end = new Date();
        let start = new Date();
